...
namespace Strider.Test.Domain.Test.Queries.Posts.QueryHandlers
{
    public class GetAllPostsQueryHandlerTest
    {
        ...
        {
            var handle = new GetAllPostsQueryHandler(_postRepository.Object);
            var response = await handle.Handle(new GetAllPostsQuery(string.Empty, 0, 0), new System.Threading.CancellationToken());
